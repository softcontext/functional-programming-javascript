...
ES6에서는 다행히 직접 구현할 필요가 없다.
Promise 객체가 이 모나드의 구현이기 때문이다.
...
반면 Promise는 구별되는 두 값을 전달하는데 하나는 성공 값이고
다른 하나는 에러를 위해 사용한다. (Either 모나드와 유사하다.)

에러는 then 메소드의 두번째 콜백으로 포착할 수 있으며
또는 이를 위해 제공되는 특별한 메소드 .catch() 를 사용할 수도 있다.
 */
