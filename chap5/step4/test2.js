...
// 조회 결과를 Maybe로 감싸고 연산명 앞에 safe 단어를 붙여 구분한다.
// Maybe.fromNullable: find 함수의 결과는 null일 수 있다.
// 결과가 있다면 Just 컨테이너에 담고 없다면 Nothing 컨테이너에 담아 리턴한다.
const safeFindObject = R.curry((db, id) => Maybe.fromNullable(find(db, id)));

const safeFindStudent = safeFindObject(DB('student'));

const address = safeFindStudent('444-44-4444')
  .map(R.prop('address')); // address: 'Seoul' 정보만 추출한다.
...
/*
  ...
  find 함수가 학생객체를 리턴하면 결과는 'Seoul' 문자열을 출력
 */
