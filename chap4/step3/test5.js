/*
  ...
  npm install log4js 로 설치하는 모듈은 https://www.npmjs.com/package/log4js 것이고,
  책에서 사용하는 log4js 모듈은 https://github.com/stritti/log4js 것이다.
  책에서 사용하는 모듈을 사용하기 위해서는
  소스 코드를 다운로드 후 다음 작업이 필요하다.
  1: cd log4js
  2: npm install
  3: grunt build // 하지만, 여기서 에러가 발생한다.
  ...
  저자가 소개하는 기술은 브라우저에서 코드가 수행됨을 전제로 하는 것을 알 수 있다.
...
// console.log(Object.keys(Log4js).filter(k=>k.includes('Appender')).sort()); // 어팬더를 확인한다.
...
const logger = function (appender, layout, name, level, message) {
  ...
  const logger = Log4js.getLogger(name); // 로거에 타이틀 설정
  logger.addAppender(myAppender); // 로거에 appender 설정
  logger.log(level, message, null); // 로거에 사용할 로깅 메소드 형식을 결정. null 자리는 exception
};

const log = R.curry(logger)('alert', 'json', 'FuncJS');
